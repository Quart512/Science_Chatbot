...
def test_search_papers_computes_paper_id_from_arxiv_id(monkeypatch):
    monkeypatch.setattr(paper_search, "arxiv_search", lambda query, max_results=5: [_fake_arxiv_result()])
...
def test_search_papers_prefers_doi_when_present(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda query, max_results=5: [_fake_arxiv_result(doi="10.1234/xyz")],
    )
...
def test_search_papers_carries_journal_ref_through(monkeypatch):
    monkeypatch.setattr(
        paper_search, "arxiv_search",
        lambda query, max_results=5: [_fake_arxiv_result(journal_ref="Phys. Rev. D 100, 1 (2024)")],
    )
...
def test_search_papers_passes_query_and_max_results_through(monkeypatch):
    ...
    assert captured == {"query": "dark matter", "max_results": 10}
