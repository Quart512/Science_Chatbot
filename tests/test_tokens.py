"""
add_tokens — models.py의 순수 헬퍼. 여러 노드에 걸쳐 tokens_used를 누적할 때 쓴다
(물리 QA·연구 워크플로우·참고문헌 추천기 공용).
...
def test_add_tokens_sums_matching_keys():
    ...
    assert add_tokens(current, new) == {"input_tokens": 13, "output_tokens": 7, "total_tokens": 20}
...
def test_add_tokens_ignores_extra_keys_in_new():
    ...
    assert add_tokens(current, new) == {"input_tokens": 1, "output_tokens": 1, "total_tokens": 2}
...
def test_add_tokens_defaults_missing_keys_to_zero():
    ...
    assert add_tokens(current, {}) == current
