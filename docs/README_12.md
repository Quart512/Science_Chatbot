...
- 2026-07-28 (4): **전문 처리 상세 설계** — 헤더 기반 섹션 분할(`pymupdf4llm` 마크다운 + `MarkdownHeaderTextSplitter`)과 길이 초과 시 점진적 분할 방침을 확정하고, 다섯 가지를 보강: ① reduce는 자유 요약이 아니라 **스키마 병합**(각 map이 같은 Pydantic 반환 → 병합·중복 제거는 파이썬 코드, merge 단계에서 사실이 망가지는 걸 방지) + 항목별 `from_section` ② 컨텍스트 예산을 **모델별로 `models.py`에 선언**(gemini·claude에선 분할이 거의 발동 안 하고 `Qwen-tuned`의 작은 `n_ctx`에서만 걸림) ③ 그래서 **단순 경로 우선**, 초과 시 조용히 자르지 말고 정직하게 실패 ④ 헤딩 탐지 실패 **폴백 2단**(섹션명 정규식 → 문단 분할) ⑤ **논문 id 정규화**(`doi:` → `arxiv:` → 내용 해시)와 재등록 시 잔여 청크 삭제. 카탈로그 기본 키를 DOI에서 `paper_id`로 변경(DOI는 nullable 컬럼).
- 2026-07-28 (3): **논문 평가 기준 정리 — 판정 대신 추출, 품질 평가는 보류**. "논문 평가"를 하나로 상정한 게 문제였음을 확인(②a·②b가 답할 질문이 다름). 과학적 타당성 판정은 피어 리뷰 영역이라 LLM에 맡기면 하류가 노이즈를 신호로 취급 — 07-15 verify 기준 수정과 같은 실패 양상. ②a는 구조화 추출(핵심 주장·근거 종류·**저자가 밝힌 한계**·미해결·preprint 여부)로, ②b는 축을 합치지 않고 병기(관련도 + `journal_ref` peer-review 여부 + 연간 인용수 + 연도, h-index 제외). **품질 평가 자체는 ⑦이 소비할 수 있게 된 시점에 재검토**(평가의 값어치가 ⑦의 활용에서 나오고, 소비처가 없으면 기준 검증도 불가). 검증 레이블은 카탈로그 `dismissed` 이력으로 확보, ① 관심사 템플릿에 "찾는 것/아는 것/제외 주제" 필드 요구. 추가로 **스트리밍 중 인터럽트(ESC 취소 + 프롬프트 대기열)를 최종 다듬기 항목으로 신설** — 취소 시 `final_answer` 미실행으로 턴 메시지가 정리되지 않는 문제, thread별 직렬화가 정확성 요구사항이라는 점을 함께 기록.
- 2026-07-28 (2): **PDF 파싱 라이브러리 선정** — PyMuPDF(+pymupdf4llm) 채택, `pdf_parse.py` 어댑터 뒤에 격리해 교체 가능하게(AGPL-3.0이라 라이선스 문제 시 pypdfium2로 전환). ML 기반 파서(marker·nougat)는 이미지 경량화 무효화로 제외, GROBID는 t4g.micro RAM 한계로 로컬 전용 옵션으로 보류. 6-3 구현 체크리스트(2단 조판 실물 확인 후 청킹 설계 / 스캔본 감지만·OCR 없음 / references 태깅 / 캡션 살리고 수식·이미지 포기) 확정. arXiv LaTeX 소스 이중 경로는 후속 과제로 기록.
- 2026-07-28: **논문 처리 3분할** (6-3 착수 전 재검토) — 기존 ②의 "abstract 트리아지 → 전문 요약·평가" 파이프라인이 성립하지 않음을 확인(유료 저널은 트리아지 통과해도 전문 못 읽음, 보유 논문은 이미 선별 완료라 트리아지 불필요). **논문 요약기(②a: 보유 논문 전문·lazy 생성·캐시) / 논문 스크리닝(②b: abstract+지표, 전문 안 읽음) / 논문 검색 어댑터**로 분리. 등록 시 인코딩(청킹·임베딩)과 요약 생성을 분리하고 논문 VDB에 `doc_type` 구분 추가, 권위 판단은 LLM이 아니라 지표 계산으로(지표 필드는 미리 스키마에 두고 비워둠), QA 중 요약 부재 시 백그라운드 생성. 6-3 범위가 ②a로 축소되고 ②b는 ③(6-6)과 함께. README 능력·데이터 표·설계 포인트, RoadMap, To Do List, architecture.png 반영.
