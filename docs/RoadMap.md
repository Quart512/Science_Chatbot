...
- [ ] **연구 워크플로우(⑥) end-to-end** — `uv run python research_workflow.py`(모듈 하단 `__main__` 스모크 테스트). 가설→설계→운영 3단계가 실제 LLM으로 이어지는지, 단계 전환이 `ainvoke({"stage": ...})` 한 번으로 되는지, `references`가 단계별로 누적되는지, 체크포인트가 파일에 남아 프로세스를 다시 띄워도 이어지는지. **지금 이 그래프를 실제로 돌려본 적이 한 번도 없다** — pytest와 `MemorySaver`뿐이다.
- [ ] **참고문헌 추천기** — 실제 arxiv + `papers_vectorstore`로. 보유 논문이 실제로 잡히는지(§B의 배율 확인과 같이), 외부 보충이 걸리는지, `reasoning`이 쓸 만한지.
- [ ] **`/equipment` 엔드포인트** — UI가 없어 **한 번도 호출된 적이 없다.** 등록→조회→부분 수정(이름만 보내고 `precautions`가 남는지)→삭제.
...
| 8월 하순 | 표면(UI) — 라이브러리에 실험도구·연구 워크플로우 탭 | ⑤⑥⑦(논문 작성) 다 나온 뒤 한 번에(사용자 결정, 08-02) — 그 시점에 Streamlit 유지 vs React 전환도 같이 판단(README "React 전환은 화면이 더 늘거나 한계에 부딪힐 때 재검토"). ⑤⑥ 백엔드는 완료(아래 완료 표) |
