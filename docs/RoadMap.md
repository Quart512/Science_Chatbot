...
| 목표 시기 | 항목 | 내용 |
|---|---|---|
| 08-09 | 추천 검색 (③) + 스크리닝 (②b) + 논문 카탈로그 | ②b: **abstract만** 보고 관련도 판정(유일한 LLM 판단) + 확인 가능한 축 **병기**(단일 점수로 합치지 않음): peer-review 여부(arXiv `journal_ref`), 연간 인용수, 출판 연도 — 전문을 읽지 않는다. 기각 이력(`dismissed`)을 기준 검증용 레이블로 축적. 논문 검색 어댑터(쿼리 → abstract+서지+지표 후보 목록)도 여기서 분리. ③: **관심사에서 트리거할 때만** 실행 (cron 아님) — 검색 → ②b 스크리닝 → 랭킹 → 카탈로그에 recommended 기록. 논문 카탈로그(SQLite, DOI 기본 키, `status: recommended/owned/dismissed`) — 등록 시 DOI 매칭으로 recommended → owned 전환(추천에서 내려감). 권위 논문 목록(인용수 기반)도 관심사별 조회·캐시. **검색·평가 내부를 공용 함수로 분리**해 참고문헌 추천기와 공유(추천기는 문맥 기반 온디맨드, QA 풀 호출은 라우터 분기). **진행 상황(07-31)**: 논문 카탈로그 RDB(`paper_catalog.py`) + 논문 검색 어댑터(`paper_search.py`, arxiv `journal_ref`/`doi` 파싱 포함) + ②b 스크리닝(`paper_screening.py`) 완료 — 아래 완료 표 참고, **권위 논문 목록은 스코프에서 뺌**(인용수 컬럼이 실제로 채워지는 외부 API 어댑터가 아직 없어 지금 만들어도 항상 빈 결과라 "단순 경로부터" 원칙에 어긋남 — 어댑터 붙을 때 재검토). 남은 것: ③ 추천 검색(검색→스크리닝→랭킹→카탈로그 기록을 엮는 오케스트레이션) → `register_paper()` 연동. **(좋은 모델로 검사 권장 — 이 프로젝트에서 데이터 무결성 위험이 가장 큰 지점. `paper_id`가 기본 키인데 VDB·카탈로그 두 저장소에 나뉘어 있고, 상태 전이(recommended→owned)와 다대다 조인이 여기서 처음 생긴다. 07-28 `retrieve()` 2k 버그처럼 "저장소가 늘면 필터가 한 군데 빠진다"가 이 저장소의 반복 실패 패턴)** |
