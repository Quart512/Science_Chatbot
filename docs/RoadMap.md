...
| 07-31 | 08-09⑤ `register_paper()` 카탈로그 연동 (08-09 완료) | `paper/paper_ingest.py`의 `register_paper()`가 정상 등록(text_extractable) 시 `paper_catalog.mark_owned(paper_id, doi=, arxiv_id=, title=, authors=, year=)`를 호출 — 추천(`recommended`)이던 논문을 사용자가 실제로 등록하면 `owned`로 전환된다(이미 있으면 upsert, 없으면 새로 생성하는 `mark_owned()` 기존 동작 그대로 사용). title/authors/year는 등록 과정에서 이미 계산해둔 `bib_meta`(화이트리스트로 거른 서지정보, 232행)를 그대로 재사용 — 새로 뽑지 않음. **스캔본(text_extractable=False)은 VDB에 아무것도 저장 안 하는 것과 대칭으로 카탈로그도 안 건드림**. 카탈로그 쓰기 실패는 arxiv 자동조회(네트워크, best-effort)와 달리 삼키지 않고 그대로 전파 — "카탈로그에 반영됨"이 이제 이 함수 계약의 일부이기 때문. **cross-id 매칭은 의도적으로 미룸**: 추천 시점엔 arxiv_id만 있어 `paper_id`가 `arxiv:XXXX`로 계산됐는데 등록 시점에 사용자가 (arxiv가 안 준) DOI를 새로 넘기면 `normalize_paper_id()`가 DOI 우선이라 `paper_id`가 바뀌어 기존 recommended 행을 못 찾고 새 행이 생기는 경우 — 라이브러리 UI(6-8)가 아직 없어 이런 입력이 실제로 어떻게 들어올지 모르는 채로 매칭 로직을 미리 만들 근거가 없음(`paper_catalog.py` 모듈 docstring에도 "실제로 걸리면 그때"로 남겨둠). 테스트 2개 추가(카탈로그 호출 인자 확인, 스캔본 스킵 확인) + 기존 테스트 전부가 실제 `data/app.db`를 안 건드리게 `mark_owned`를 기본 no-op으로 스텁하는 autouse fixture 추가. **이걸로 08-09(추천 검색+스크리닝+카탈로그) 전체 완료** |
