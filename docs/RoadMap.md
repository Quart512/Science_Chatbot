...
| 8월 하순 | 연구 워크플로우 (⑥) | 실험도구 DB(⑤)는 백엔드 완료(위 완료 표, UI는 ⑤⑥⑦ 다 나온 뒤 한 번에). **핵심 설계 — 단계 전환은 START 라우팅**(08-02): 처음엔 모든 노드를 자동으로 잇는 단일 체인이었는데, 참고문헌 노드가 "확인하고 재생성 여부를 판단하라"는 comment를 남겨도 그래프가 다음 단계까지 자동 실행해버려 검토할 틈이 없다는 문제를 발견 — 단계 사이(가설→설계→운영) 전환은 전부 사람이 트리거해야 함. `aget_state`→파이썬 함수 직접 호출→`aupdate_state` 방식(글루 함수)을 1차로 짰다가, **"그냥 START에서 라우팅하면 안 되냐"는 사용자 지적을 토이 그래프로 실제 검증한 뒤** 채택 — `WorkflowState.stage`(`"hypothesis"`/`"design"`/`"operation"`)를 `route_by_stage()`가 보고 START에서 분기, 각 단계는 `[생성 노드] → [참고문헌 노드] → END`로 끝난다. 사람이 "다음 단계로"는 `ainvoke({"stage": "design", ...새 입력}, config)` **한 번**으로 충분(별도 `aupdate_state` 불필요 — 실제 확인함), 이전 단계 값도 체크포인트에서 그대로 보임. 이 방식의 이점: 모든 단계가 그래프 엔진(스트리밍·트레이싱)을 균일하게 타고, "재설계 필요" 판정이 나오면 사람이 stage를 "design"으로 되돌리기만 하면 되므로 되돌아가는 엣지가 따로 필요 없음. **3단계 완성**: ① `generate_hypothesis`(주제→가설), ② `design_experiment`(가설+`equipment.py` 보유 장비 목록[짧으니 프롬프트에 통째로]→구조화 설계, README의 Plan-and-Execute는 분리 안 함 — 산출물 자체가 설계 문서라 "실행"이 따로 없어서), ③ `analyze_results`(가설·예측·절차 + **사용자가 실제로 실험하고 입력한 결과**를 비교해 분석+재설계 필요 여부 — LLM이 실험을 대신 하는 게 아니라 사람이 한 결과만 분석). 각 단계 뒤에 클로저 팩토리 `_make_reference_node(get_text, stage_name)`(`tool.py`의 `make_search_tool`과 같은 패턴)로 찍어낸 참고문헌 노드가 붙어 `WorkflowState.references`에 `paper_id` 기준 중복 없이 누적 — `screen_candidate()`를 관심사 dict 대신 `topic: str`로 리팩터해 ③ 추천 검색과 `reference_recommender.py`가 공유(가짜 interest 객체 없이). "이미 증명된 이론인지"는 LLM이 판정 안 하고(판정 대신 추출/신호 원칙) `WorkflowState.comment`(graph.py의 comment와 같은 결정론적 채널)로 사람이 참고문헌 보고 판단하도록만 안내. 테스트: 노드 24개 + 3단계 전체를 실제 컴파일된 그래프로 따라가는 통합 테스트 2개(`MemorySaver`) — 전체 260개 통과. **남은 것**: 안전 가드레일(장비 안전 규칙 조회 + 임계치 초과 시 승인 필요 — **좋은 모델로 검사 권장**, 여기가 `interrupt_before`가 실제로 필요할 수 있는 유일한 후보 지점) → 표면(UI) |
