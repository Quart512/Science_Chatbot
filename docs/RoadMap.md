...
- [ ] **연구 워크플로우(⑥⑦) end-to-end** — `uv run python research_workflow.py`(모듈 하단 `__main__` 스모크 테스트). 가설→설계→운영→보고서→논문 초안 5단계가 실제 LLM으로 이어지는지, 단계 전환이 `ainvoke({"stage": ...})` 한 번으로 되는지, `references`가 단계별로 누적되는지, 체크포인트가 파일에 남아 프로세스를 다시 띄워도 이어지는지. **지금 이 그래프를 실제로 돌려본 적이 한 번도 없다** — pytest와 `MemorySaver`뿐이다.
