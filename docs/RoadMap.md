...
| 목표 시기 | 항목 | 내용 |
|---|---|---|
| 다음 세션 | 연구 워크플로우 — 논문 초안 인앱 편집 | 08-04 체크포인트 히스토리·복원 작업 직후 사용자가 요청, 순서만 뒤로 미룸(사용자 지시: "끝내고 다음에 이어서 가자"). `draft_paper()`가 채우는 `title`/`abstract`/`introduction`/`methods`/`results`/`discussion`은 PDF가 아니라 평범한 텍스트 필드라 편집 자체는 어렵지 않음. `knowledge_notes.py`의 편집기 **코드**는 그대로 재사용하기 어렵다(독립된 RDB 테이블+벡터DB 재색인이 딸려있어, 초안을 거기로 복사해 편집하면 `citations`/`[CITE:paper_id]` 구조와 안 이어짐) — 대신 **패턴만**(`notes.py`처럼 필드별 `st.text_area`+저장 버튼) 재사용하고, 저장은 08-04에 쓴 것과 같은 `aupdate_state(..., as_node="__start__")`(LLM 재호출 없이 값만 덮어쓰기)로 새 엔드포인트를 만들 예정 |
