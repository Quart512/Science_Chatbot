...
| 8월 하순 | 연구 워크플로우 (⑥) | 실험도구 DB(⑤)는 백엔드 완료(위 완료 표, UI는 ⑥⑦ 다 나온 뒤 한 번에). **현재 그래프**: `START → generate_hypothesis → find_hypothesis_references → design_experiment → find_design_references → END`(`research_workflow.py`, 오케스트레이터와 State 무관한 독립 그래프·체크포인트 `data/research_workflow_checkpoints.sqlite`). 부품 쪽: `screen_candidate()`를 관심사 dict 대신 `topic: str`로 리팩터해 ③ 추천 검색과 새 모듈 `reference_recommender.py`(텍스트→검색어 추출→보유 VDB 우선→부족하면 신규 검색+스크리닝)가 공유. 참고문헌 노드는 클로저 팩토리 `_make_reference_node(get_text, stage_name)`로 찍어냄(`tool.py`의 `make_search_tool`과 같은 패턴) — `WorkflowState.references`에 `paper_id` 기준 중복 없이 누적, `added_by_stage`로 어느 단계가 추가했는지 기록. `design_experiment`는 가설+`equipment.py` 보유 장비 목록(짧으니 프롬프트에 통째로)으로 LLM 호출 1회에 독립/종속/통제변수+장비+절차 구조화 출력 — README의 "Plan-and-Execute"는 분리 안 함(08-02, 사용자 판단 — 이 단계 산출물 자체가 설계 문서라 "실행"이 따로 없어서, 필요해지면 나중에 멀티스텝으로 확장). **"이미 증명된 이론·이미 한 실험인지" 확인은 HITL이 아니라 사람 판단 재생성으로(08-02, 사용자 판단)**: `interrupt_before` 없이 그냥 노드를 사용자 피드백과 함께 다시 invoke하면 됨(07-31 "문서 작성기" 폐기와 같은 교훈 — "이렇게 고쳐줘"는 실행 중단·재개가 아니라 평범한 재요청). LLM이 "선행 연구 여부"를 판정하지도 않음(판정 대신 추출/신호 원칙) — 참고문헌 노드가 새로 찾은 게 있으면 `WorkflowState.comment`(graph.py의 comment와 같은 결정론적 채널)에 "참고논문 확인해서 선행 연구 있는지 보고 템플릿을 채우거나 수정, 부족하면 재생성" 안내를 채워 사람이 판단하게 함(못 찾았거나 전부 중복이면 재생성 권유로 문구가 바뀜). 테스트: 노드 8개 파일에 걸쳐 21개(맨 함수 호출, 몽키패치) — 전체 252개 통과. **남은 것**: 실험 운영 노드 → 재실험 루프(실험 설계로 되돌아가는 조건부 엣지) → 안전 가드레일(장비 안전 규칙 조회 + 임계치 초과 시 승인 필요, `interrupt_before`가 실제로 필요한 유일한 지점 — **좋은 모델로 검사 권장**, 우회 경로 있으면 가드레일이 있는 척만 하는 게 됨) → 표면(UI, ⑤⑥⑦ 다 나온 뒤 한 번에) |
