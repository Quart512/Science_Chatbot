...
물리 연구 어시스턴트 챗봇의 개발 이력과 계획. 상세 회고는 [README_08.md](README_08.md), [README_09.md](README_09.md), [README_10.md](README_10.md), [README_11.md](README_11.md), [README_12.md](README_12.md), [README_13.md](README_13.md) 참고.
...
| 8월 하순 | 연구 워크플로우 (⑥) | 실험도구 DB(⑤)는 백엔드 완료(위 완료 표, UI는 ⑥⑦ 다 나온 뒤 한 번에). 목표 형태: 가설 수립 → 실험 설계(Plan-and-Execute) → 실험 운영을 별도 표면(단계형 화면)으로, 안전 가드레일 `interrupt_before` HITL(재실험 루프는 실험 설계만 재호출), 각 단계가 참고문헌 추천기를 호출해 워크플로우 공유 references 목록에 누적(서지+인용 이유+추가 단계). **진행 상황(부품부터 하나씩)**: 1단계(완료) `paper_screening.screen_candidate()`를 관심사 dict 대신 `topic: str`로 리팩터 — ③ 추천 검색과 참고문헌 추천기가 같은 스크리닝 함수를 공유하되 관심사를 몰라도 되게(가짜 interest 객체 안 씀, `paper_recommend.py`는 `_interest_topic_text()`로 4필드를 텍스트 조립해 넘김). 2단계(완료) 새 모듈 `reference_recommender.py` — `extract_search_query(text)`(LLM 1회) → 보유 논문 VDB 우선(스크리닝 없이 채택) → 부족하면 `paper_search`+`screen_candidate()`로 신규만 스크리닝, 보유 먼저·신규 뒤(축 안 합침). 아직 호출하는 곳 없음(부품만 먼저). 3단계(완료) 새 그래프 `research_workflow.py` — 오케스트레이터와 State 공유 안 하는 독립 최상위 그래프(체크포인트도 `data/research_workflow_checkpoints.sqlite`로 분리), 첫 노드 `generate_hypothesis`(주제 → 검증 가능한 가설+근거+예측, `model`/`disabled_models` 서킷 브레이커는 orchestrator.ParentState와 같은 패턴). 4단계(완료) `WorkflowState.references`(누적 목록: `paper_id`/`title`/`source`/`reasoning`/`added_by_stage`) + 노드 `find_hypothesis_references` — 방금 나온 가설 문장으로 `reference_recommender.recommend_references()` 호출, 이미 목록에 있는 `paper_id`는 건너뛰고(먼저 추가한 단계 표시 유지) 새로 찾은 것만 `added_by_stage="hypothesis"`로 누적. 실패해도 이 단계만 건너뜀(가설은 이미 나왔으므로 워크플로우 전체를 막을 이유 없음 — 삭제된 자동 제안 훅이 쓰던 것과 같은 논리). 그래프: `START → generate_hypothesis → find_hypothesis_references → END`. 테스트: 1단계 회귀 1개, 2단계 8개, 3+4단계 8개(노드 맨 함수 호출) — 전체 242개 통과. **남은 것**: 실험 설계 노드(Plan-and-Execute, 자기 참고문헌도 같은 방식으로 누적) → 실험 운영 노드 → 안전 가드레일 `interrupt_before`(**좋은 모델로 검사 권장** — 실제로 "사람 승인 전까지 진행 불가"를 보장하는지, 우회 경로가 하나라도 있으면 가드레일이 있는 척만 하는 게 됨. 며칠씩 걸리는 상태 있는 워크플로우라 재시작·재실험 루프에서 상태가 어긋나기도 쉬움) → 표면(UI) |
