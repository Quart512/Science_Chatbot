...
| 8월 하순 | 연구 워크플로우 (⑥) | 실험도구 DB(⑤)는 백엔드 완료(위 완료 표, UI는 ⑥⑦ 다 나온 뒤 한 번에). 목표 형태: 가설 수립 → 실험 설계 → 실험 운영을 별도 표면(단계형 화면)으로, 안전 가드레일 `interrupt_before` HITL(재실험 루프는 실험 설계만 재호출), 각 단계가 참고문헌 추천기를 호출해 워크플로우 공유 references 목록에 누적(서지+인용 이유+추가 단계). **진행 상황(부품부터 하나씩, 그래프: `START → generate_hypothesis → find_hypothesis_references → design_experiment → find_design_references → END`)**: ① `screen_candidate()`를 관심사 dict 대신 `topic: str`로 리팩터(③ 추천 검색과 참고문헌 추천기가 스크리닝 함수 공유, 가짜 interest 객체 안 씀) → ② 새 모듈 `reference_recommender.py`(텍스트→검색어 추출→보유 VDB 우선→부족하면 신규 검색+스크리닝, 보유 먼저·신규 뒤) → ③ 새 그래프 `research_workflow.py` 신설(오케스트레이터와 State 무관한 독립 그래프, 체크포인트도 `data/research_workflow_checkpoints.sqlite`로 분리), `generate_hypothesis` 노드 → ④ `WorkflowState.references` 누적 목록 + `find_hypothesis_references` 노드(참고문헌 추천기를 실제로 호출하는 첫 사례, paper_id로 중복 방지, 실패해도 이 단계만 건너뜀) → ⑤ `design_experiment` 노드(가설+`equipment.py` 보유 장비 목록을 프롬프트에 통째로 넣어 — 실험도구 DB 착수 시 정한 "목록 짧으면 프롬프트에" 방식을 실제로 씀 — LLM 호출 1회로 독립/종속/통제변수+필요장비+절차 구조화 출력) + `find_design_references`(참고문헌 노드를 클로저 팩토리 `_make_reference_node(get_text, stage_name)`로 일반화 — `tool.py`의 `make_search_tool`과 같은 패턴, ④의 노드도 이 팩토리로 재구현). **Plan-and-Execute 분리 안 함(08-02, 사용자 판단)**: README상 이 단계 핵심 기법은 Plan-and-Execute(계획자+실행자+재계획 멀티스텝 패턴)였지만, 이 단계 산출물 자체가 설계 문서라 "실행"이 따로 없음(행동은 다음 실험 운영에서 일어남) — 그래서 LLM 호출 한 번으로 단순화, 필요해지면 나중에 멀티스텝으로 확장. 테스트: ①~④ 17개, ⑤ 15개(노드 맨 함수 호출, 몽키패치) — 전체 249개 통과. **남은 것**: 실험 운영 노드 → 재실험 루프(실험 설계로 되돌아가는 조건부 엣지) → 안전 가드레일 `interrupt_before`(**좋은 모델로 검사 권장** — 실제로 "사람 승인 전까지 진행 불가"를 보장하는지, 우회 경로가 하나라도 있으면 가드레일이 있는 척만 하는 게 됨. 며칠씩 걸리는 상태 있는 워크플로우라 재시작·재실험 루프에서 상태가 어긋나기도 쉬움) → 표면(UI) |
