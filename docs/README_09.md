...
verify마저 qwen으로 진행하는 실험 진행

4. bare Qwen vs graph(Qwen+verify as claude) vs graph(Qwen+verify as Qwen)
- graph(Qwen+verify as Qwen)
=== 평가 요약 ===
전체: 평균 0.176 (n=31)
일반 문항: 평균 0.182 (n=28)
미해결 문항(인정+사실정확성): 평균 0.117 (n=3)

카테고리별 평균:
  atomic: 평균 0.320 (n=5)
  electromagnetism: 평균 0.138 (n=4)
  mechanics: 평균 0.090 (n=5)
  open_problem: 평균 0.117 (n=3)
  quantum: 평균 0.310 (n=5)
  relativity: 평균 0.130 (n=5)
  thermodynamics: 평균 0.075 (n=4)

{'eval_qwen-tuned-q4.json': 0.13225806451612898, 
'eval_graph-Qwen.json': 0.44516129032258067, 
'eval_graph-Qwen-only.json': 0.1758064516129032}

구조적인 변경(ReAct+Evaluator-Optimizer Pattern)만으로 증가한 평가점수는
0.132->1.176 정도였다.
evaluator(verify)를 claude로 썼을 때 0.44로 증가하는 모습을 보였다.
아예 claude가 정답을 알려주는 문항도 있었던 것으로 볼 때 verify의 영향이 크다고 판단된다.

## 평가 결과 - eval_claude vs graph(claude)
gemini 2.5 flash는 사용량 제한으로 진행하지 못했음
1. eval_claude vs graph(claude)
...
전체: 평균 0.827 (n=31)
일반 문항: 평균 0.840 (n=28)
미해결 문항(인정+사실정확성): 평균 0.707 (n=3)

카테고리별 평균:
  atomic: 평균 0.910 (n=5)
  electromagnetism: 평균 0.700 (n=4)
  mechanics: 평균 0.870 (n=5)
  open_problem: 평균 0.707 (n=3)
  quantum: 평균 0.804 (n=5)
  relativity: 평균 0.890 (n=5)
  thermodynamics: 평균 0.838 (n=4)

  0.813-> 0.827로 올라갔지만 여전히 0.904에는 못미친다.
  평가를 claude로 하는 와중에 claude 모델만으로 돌아가는 claude evaluate가 평가가 좋을수밖에 없는 반면 graph는 일차적으로는 gemini로 돌아가고, 사용량을 다 쓰면 claude로 돌아가기 때문에, 납득이 가는 결과이다. 다만 gemini가 0.813 이하, 적어도 0.827 이하로 나온다면 구조가 성능을 개선시킴을 확인할 수 있다.
  gemini2.5 flash만으로 전체 문항을 다 돌리고, 나중에 eval을 하는 gemini-only test를 해볼 필요가 있다.
## 장애 복원력 테스트
...
이후에 verify 단계에서 모든 모델을 소모할 때, 에러로 실행을 터트리는 것이 아니라 verify를 생략하는 브랜치를 추가했다.
