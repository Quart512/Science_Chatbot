...
| graph(claude 고정, gemini 혼입 제거) | 0.910 | bare와 거의 동일 — 남은 차이는 "측정 문제" 단일 문항 (실험 4) |
| bare claude-haiku | 0.915 | |
| **graph(claude 고정) + verify 기준·출력 이원화** | **0.926** | **bare claude 최초 역전** (실험 5 수정 반영) |

> **신뢰도 단서**: 각 구성은 1회씩만 측정한 단일 실행 결과다 (반복 실험·평균 아님). LLM 생성·채점의 비결정성을 감안하면 ±0.01 수준의 차이는 단정할 수 없다. 다만 0.813 → 0.827 → 0.910 → 0.926으로 수정마다 일관되게 오른 추세, 그리고 약한 모델 구성(0.132→0.445)과의 격차는 단일 실행으로도 유의미하다.

카테고리별:

| 카테고리 | bare Qwen | graph+Qwen (self-verify) | graph+Qwen (verify=claude) | graph 기본 (개선 후) | graph 최종 (이원화 후) |
|---|---|---|---|---|---|
| atomic | 0.210 | 0.320 | 0.410 | 0.910 | 0.950 |
| electromagnetism | 0.150 | 0.138 | 0.575 | 0.700 | 0.943 |
| mechanics | 0.190 | 0.090 | 0.720 | 0.870 | 0.910 |
| open_problem | 0.150 | 0.117 | 0.583 | 0.707 | 0.907 |
| quantum | 0.100 | 0.310 | 0.380 | 0.804 | 0.944 |
| relativity | 0.090 | 0.130 | 0.380 | 0.890 | 0.930 |
| thermodynamics | 0.025 | 0.075 | 0.075 | 0.838 | 0.885 |
| **전체** | **0.132** | **0.176** | **0.445** | **0.827** | **0.926** |
...
> graph(claude 고정): judge temp 고정 전 0.897 → temp=0 재측정 **0.910** (bare 0.915와 사실상 동률, solved는 양쪽 다 0.917로 동일). 남은 격차는 전부 unsolved에서 나왔다.

unsolved 3문항을 개별로 보면 원인이 훨씬 좁게 좁혀진다:
...
요약:
evaluate 방식이 미리 정한 정답과의 유사도를 측정하는 방식이라, "알겠습니다. 수정하여 답변하겠습니다" 같은 프리앰블이 유사도를 깎아먹는다는 걸 재확인했다.(루프를 많이 타는 graph 특성상 bare에 비해 프리앰블 생성될 가능성 큰 것으로 보임) 
처음엔 프리앰블을 정규식 등으로 걸러내거나 지우려 시도했지만 안정적으로 못 걸러냈다(실패). 
관점을 바꿔보니 — 사용자 입장에선 오히려 이런 프리앰블·자기 평가가 "top_k를 늘리거나 모델을 바꿔볼까" 판단에 도움이 되는 정보다. 그래서 "지우기"가 아니라 "채널을 분리해서 평가에서만 빼기"로 방향을 틀었다. 그 결과, ver0.4가 bare claude의 평가를 소폭 앞질렀다.
...
3. **출력 이원화** — 사용자에게는 answer(본문)와 comment(부가 정보)를 모두 주고, **평가는 answer만** 채점:
   ...
   - **시스템 comment**는 LLM이 아니라 코드가 작성 — `generate`/`run_tools`/`verify` 매 노드가 지날 때마다 `state.comment`에 f-string으로 계속 이어붙인다(단발성 요약이 아니라 실행 트레이스 누적): 몇 번째 generate 시도인지, tool 호출 결과(사용량 한도 초과·호출 실패 등 에러 메시지 포함), 이번 라운드에 새로 disabled된 모델, 몇 번째 verify에서 통과/반려됐는지, verify가 남긴 comment까지 — 마지막엔 limit 도달 시 "남은 문제점 + top_k/모델 재시도 권장" 문구도 덧붙음
...
**최종 재평가 (수정 전부 반영, 31문항)**: graph(claude 고정) **0.926** — **bare claude(0.915)를 처음으로 역전.** 하락의 원인이 파이프라인 원리가 아니라 측정 오염(프리앰블·실패 고지)과 verify 오판(문서 근거성을 반려 사유로)이라는 **구현 문제**였음이 확정됐다. 카테고리별로는 근접-오검색과 반려 연쇄로 무너지던 곳들이 크게 회복: electromagnetism 0.700→0.943, open_problem 0.707→0.907. 단일 실행이라 신뢰도 단서(종합 비교 참고)는 붙지만, 실험 3에서 시작한 "구조가 강한 모델을 개선할 수 있는가"라는 질문에 긍정 답이 나왔다.
...
- **에이전트 구조의 효과는 베이스 모델의 강함에 반비례**: 약한 모델(Qwen)은 구조+좋은 verifier로 3.4배 상승(0.132→0.445), 강한 모델(claude)은 처음엔 오히려 하락(0.905→0.827) — 파이프라인은 공짜가 아니고, 저품질 컨텍스트 주입 경로이기도 하다. **단, 측정 오염과 verify 오판을 고치자 강한 모델에서도 역전(0.915→0.926)**: 하락은 원리가 아니라 구현의 문제였다
...
- gemini-only 테스트 (쿼터 리필 후) — bare gemini vs graph(gemini)에서도 역전이 재현되는지 확인
- 반복 실행으로 0.915 vs 0.926 차이의 통계적 신뢰 확보 (3회 이상 평균)
...
- 학습 데이터 비율 조정: 논문(문어)만으로는 대화능력 미묘 → 래딧 평서문 등 혼합 비율 실험
- 비교 평가 시스템: ① 자기 모델 ② 젬마 4 e2b(가중치 공개) ③ 제미나이 2.5 플래시(프론티어) — retrieval 포기하더라도 prompt/model/parser 구성으로 비교 (평가셋 31문항)
- 프론티어 모델로 진도 실습(RAG, LangGraph) 후 내 모델로 재현 확인
- 튜닝·서빙 배우면 AI 서버 띄우고 운영해볼 것
