...
- **다음 작업 (To Do List "이번 주" 최상단)**: 과학챗봇 3 — 단기기억 + 쓰레드. `MemorySaver` checkpointer 도입, `thread_id`를 FastAPI 요청 필드(`Query`)에 추가해 유저별 세션(멀티턴 대화)을 분리. 지금 `main.py`의 `/query`는 매 요청이 독립적인 단발성 호출이라 이전 대화 맥락이 없는 상태 — thread_id 기반 분리가 들어오면 이번 과제에서 본 "요청 1건당 프로세스가 어떻게 반응하는가"의 관찰이, "같은 프로세스가 여러 쓰레드/세션을 어떻게 구분해 처리하는가"로 확장됨.
