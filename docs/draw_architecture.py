...
--- 배치 규칙 (왜 이렇게 놓았는가) ---
1. **라이브러리를 가운데 둔다.** 라이브러리의 기능(논문 요약기·논문 찾기·등록 폼
...
box(66, 106, 65, 73, "논문 요약기", "전문을 읽고 핵심 주장·근거·한계 정리",
...
# ── 화면 → 기능
arrow((25, 84.1), (25, 74.4))            # 메인 챗 → 물리 QA
...
arrow((65.6, 69), (52.4, 69), color=C_SHARE_ARROW, lw=1.7)      # 논문 요약기 → 물리 QA
...
# 등록 폼 채우기 → 메인 챗: 직선으로는 물리 QA를 관통하므로 왼쪽 복도(x=53.5)로 돌린다
...
arrow((22, 61.6), (22, 16.9), color=C_STORE_ARROW, lw=1.5)      # 물리 QA → 파인만 강의록
arrow((46, 61.6), (52, 16.9), color=C_STORE_ARROW, lw=1.5)      # 물리 QA → 논문 내용
arrow((65.6, 67), (60, 16.9), color=C_STORE_ARROW, lw=1.5)      # 논문 요약기 → 논문 내용·목록
...
# 논문 요약기 → library/ 폴더: 아래로 곧장 내리면 논문 찾기·등록 폼을 관통하므로
