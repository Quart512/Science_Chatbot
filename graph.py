...
class State(TypedDict):
    ...
    disabled_tools: list #[] — 서킷 브레이커로 제외된 tool 이름들
...
def route_after_generate(state: State) -> Literal["tools", "verify"]:
...
def run_tools(state: State) -> dict:
    last = state["messages"][-1]
    failures = dict(state.get("tool_failures", {}))
    disabled = list(state.get("disabled_tools", []))
...
graph.add_edge("tools", "generate")   # tool 결과 들고 generate로 복귀 (ReAct 루프)
