...
### 능력 — 화면 뒤에서 실제 일을 하는 기능들

사용자가 직접 대화하는 대상은 메인 챗 하나뿐이다. 아래는 그 챗 안에서, 또는 라이브러리·연구 워크플로우 화면의 버튼·폼을 눌렀을 때 그 순간 호출돼 일을 하고 끝나는 기능들이다 — 각각이 무엇을 해주는지만 간단히 적는다(내부 구조·설계 이유는 아래 "설계 포인트"·`docs/RoadMap.md` 참고).
...
| ~~피드 수집~~ (아직 없음) | 관심 주제와 무관하게 요즘 화제가 되는 논문·소식을 모아 보여주는 기능. 아이디어만 있고 지금은 우선순위에서 밀려 있다 | — |
| 번역 레이어 (아직 없음) | 답변을 자연스러운 한국어로 한 번 더 다듬어주는 후처리. 아직 만들어지지 않았다 | — |
...
- **프론트 스택 — React+TypeScript로 전환(08-04)**: Streamlit의 구조적 한계(페이지 단위 전체 재실행이라 여러 화면 동시 사용 불가, Cmd+C 복사 시 리런되는 버그)에 실사용 중 부딪혀 전환 착수·1차 완료(`frontend-react/`, Vite SPA). 왼쪽 네비게이션 + 가운데 라우팅 콘텐츠 + 오른쪽 **항상 떠 있는 챗 패널** 3분할 셸(다른 화면을 보면서 챗을 동시에 쓸 수 있음). 기존 `frontend/`(Streamlit)는 기능이 전부 이관돼 08-04에 삭제(싱글 유저 로컬 앱 패키징을 앞두고 중복 프론트를 배포판에 실을 이유가 없어짐) — 상세: `docs/RoadMap.md` "프론트 스택 전환" 설계 노트.
