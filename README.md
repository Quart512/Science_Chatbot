...
| 문서 작성기 (①⑤ 공용) | 대화 → 템플릿 초안(구조화 출력) → 폼에서 확인·수정 후 저장. **`interrupt` 기반 HITL은 07-31에 폐기**(초안 확인·수정은 이미 정상적인 멀티턴 대화·평범한 폼 제출이라 실행 중단·재개가 애초에 불필요했음) — 지금은 두 트리거로 존재: 메인 챗의 자동 제안 훅(대화가 반복되면 제안, `should_suggest` 게이트) + 표면의 명시적 버튼(사용자가 누른 것 자체가 의도 신호라 게이트 없이 곧장 초안 생성) | 구조화 출력 초안 생성 + 폼 확인 (HITL 아님) | 메인 챗(자동 훅) · 라이브러리(버튼) — 템플릿만 갈아끼움(관심사/실험도구) |
...
- **관심사 등록은 두 경로 — 자동 제안 훅 + 명시적 버튼**: (1) 챗 그래프의 final_answer 뒤에서 싼 모델로 1회 판정해 반복되는 주제만 제안(스팸 방지, `should_suggest` 게이트). (2) 라이브러리의 "관심사로 등록" 버튼 — 사용자가 명시적으로 누른 것 자체가 의도 신호라 `should_suggest` 판정 없이 대화 이력에서 곧장 초안을 뽑는다. 두 경로 다 저장은 같은 `POST /interests`.
